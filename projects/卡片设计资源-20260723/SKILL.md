...
```bash
node ~/.hermes/skills/ljg-card/assets/capture.js <html> <png> <width> <height> [fullpage]
```

依赖：`~/.hermes/skills/ljg-card/node_modules/` 中的 playwright。如报错：

```bash
cd ~/.hermes/skills/ljg-card && npm install playwright && npx playwright install chromium
```

### Footer

- 左侧：logo + Fisher（已硬编码在模板中）
...
当需要一次性生成多张词汇/话术学习卡片时：

```bash
# 并行生成 + 截图
node ~/.hermes/skills/ljg-card/assets/capture.js /tmp/card_01.html ~/Downloads/card_01.png 1080 900 fullpage &
node ~/.hermes/skills/ljg-card/assets/capture.js /tmp/card_02.html ~/Downloads/card_02.png 1080 900 fullpage &
wait
```

**WeChat 发送限流**：每发3-4张后需等待约60秒。先发图片再补文字，避免触发 cooldown。
